...
''' % self.filename
    ...
    def test_rename(self):
        with redirected_io() as stdout:
            main(['rename',  '--dry-run', '{tracknumber:02} {artist} - {album} - {title}', self.filename])
...
class TestCrossfire(TestCase):
    ...
    def test_rename(self):
        with redirected_io() as stdout:
            main(['rename', '--dry-run', '--verbose', '{artist}', self.filename, self.foo])
            assert stdout.getvalue() == """'%s'  ==>  'Test Artist.m4a'
Skipping %s: unknown extension: .foo\n""" % (self.filename, self.foo)
    ...
    def test_update_trackstart(self):
        with redirected_io() as stdout:
            main(['update', '--verbose', "--dry-run", "--artist=Alice", '--trackstart=10', self.filename, self.original])
            assert stdout.getvalue() == """Update tags for %s:
tracknumber: 10
artist: Alice
Update tags for %s:
...
def test_help_missing():
    with pytest.raises(SystemExit) as excinfo:
        main(['help', 'non-exist'])
    assert excinfo.exconly() == "SystemExit: 'non-exist' is not a tag command. See 'tag help'."
